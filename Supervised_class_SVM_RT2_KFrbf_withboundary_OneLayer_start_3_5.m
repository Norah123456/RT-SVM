...
diary('DiarySVMSamplingTest1_1000_3_5_OneLayer.txt');
diary on
...
ny_start=ny*3/5;
sample1(:,1)=reshape(datav0_innerc(:,ny_start,:),[],1);
sample1(:,2)=reshape(datav1_innerc(:,ny_start,:),[],1);
sample1(:,3)=reshape(datav2_innerc(:,ny_start,:),[],1);
sample1(:,4)=reshape(datav3_innerc(:,ny_start,:),[],1);

label_SVMclassifier1=reshape(data_class1_innerc(:,ny_start,:),[],1);
...
filename = ['classification_' num2str(i) '_RT2_supervised_resampled_SVMrbf_withboundary_allcentroid_Transformed180_OneLayer.nc'];
voxelsize = 1.0;
...
writecell(result,['rock_type_' num2str(r) '_RT' num2str(k) '_SVM_3_5_Transformed180_OneLayer.xls']);
